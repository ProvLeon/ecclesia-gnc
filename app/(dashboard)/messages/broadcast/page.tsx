...
export default function BroadcastPage() {
  ...
  const [type, setType] = useState<'all' | 'status' | 'department'>('all')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await sendBroadcast({
        type,
        memberStatus: type === 'status' ? memberStatus : undefined,
        message,
      })
      if (result.success) {
        setSuccess(true)
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="border-slate-200 dark:border-slate-700">
          ...
          <CardContent>
            <RadioGroup value={type} onValueChange={(value) => setType(value as 'all' | 'status' | 'department')} className="space-y-3">
              <div className="flex items-center space-x-3 p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800">
                ...
                <Label htmlFor="all" className="flex-1 cursor-pointer">
                  ...
                  <p className="text-sm text-slate-500">Send to all active members with a phone number</p>
                </Label>
              </div>
              <div className="flex items-center space-x-3 p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800">
                <RadioGroupItem value="status" id="status" />
                <Label htmlFor="status" className="flex-1 cursor-pointer">
                  <span className="font-medium">By Member Status</span>
                  <p className="text-sm text-slate-500">Filter by specific member status</p>
                </Label>
              </div>
              {type === 'status' && (
                <div className="ml-8 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                  <RadioGroup value={memberStatus} onValueChange={setMemberStatus} className="space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="visitor" id="status-visitor" />
                      <Label htmlFor="status-visitor" className="cursor-pointer">Visitor</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="new_convert" id="status-new-convert" />
                      <Label htmlFor="status-new-convert" className="cursor-pointer">New Convert</Label>
                    </div>
                  </RadioGroup>
                </div>
              )}
