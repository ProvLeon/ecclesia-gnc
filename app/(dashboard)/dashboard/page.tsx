...
export default async function DashboardPage() {
  ...
  // Permissions checks
  const canViewMembers = hasPermission(user.role, 'members:view')
  const canViewFinance = hasPermission(user.role, 'finances:view')
  const canViewAttendance = hasPermission(user.role, 'attendance:view')
  const canViewShepherding = hasPermission(user.role, 'shepherding:view')

  // Quick Actions checks
  const canCreateMember = hasPermission(user.role, 'members:create')
  const canRecordFinance = hasPermission(user.role, 'finances:create')
  const canRecordAttendance = hasPermission(user.role, 'attendance:record')
  const canSendSMS = hasPermission(user.role, 'sms:send')
  const canCreateFollowUp = hasPermission(user.role, 'shepherding:create')
  ...
  return (
    <main className="space-y-6 pb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Quick Actions - Only show if at least one action is available */}
        {(canCreateMember || canRecordFinance || canRecordAttendance || canSendSMS || canCreateFollowUp || canViewMembers) && (
          <Card className="lg:col-span-1 border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
            ...
            <CardContent className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 gap-2 pt-4">
              {canCreateMember && <QuickAction icon={Users} label="Add Member" href="/members/new" />}
              {canRecordFinance && <QuickAction icon={Wallet} label="Record Tithe" href="/finance/tithe/new" />}
              {canRecordAttendance && <QuickAction icon={CalendarCheck} label="New Service" href="/attendance/new" />}
              {canSendSMS && <QuickAction icon={MessageSquare} label="Send SMS" href="/messages/compose" />}
              {canCreateFollowUp && <QuickAction icon={Heart} label="Schedule Follow-up" href="/shepherding/new" />}
              {canViewMembers && <QuickAction icon={Building2} label="Departments" href="/departments" />}
            </CardContent>
          </Card>
        )}
      ...
      {canViewShepherding && stats.pendingFollowups > 0 && (
