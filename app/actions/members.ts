...
import { members, memberDepartments, departments } from '@/lib/db/schema'
...
export async function getMembers(filters: MemberFilters = {}) {
    ...
    if (status && status !== 'all') {
        conditions.push(eq(members.memberStatus, status as any))
    }

    if (gender && gender !== 'all') {
        conditions.push(eq(members.gender, gender as any))
    }
    ...
    const orderColumn = members[sortBy as keyof typeof members] || members.createdAt
